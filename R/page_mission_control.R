  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$project_health <- renderUI({
      ...
      ui_health_summary(
        ...
        ui_status_tile("Analytical Campaigns", paste(state$campaigns$total_campaigns[[1]] %||% 0L, "campaigns"), status = if ((state$campaigns$knowledge_conflicts[[1]] %||% 0L) > 0L || (state$campaigns$blocked_campaigns[[1]] %||% 0L) > 0L) "error" else if (((state$campaigns$knowledge_weakened[[1]] %||% 0L) + (state$campaigns$knowledge_superseded[[1]] %||% 0L) + (state$campaigns$awaiting_approval[[1]] %||% 0L) + (state$campaigns$awaiting_adoption[[1]] %||% 0L)) > 0L) "warning" else if ((state$campaigns$active_campaigns[[1]] %||% 0L) > 0L) "info" else if ((state$campaigns$completed_campaigns[[1]] %||% 0L) > 0L) "success" else "neutral", detail = paste(ui_status_label(state$campaigns$closure_recommendation[[1]] %||% state$campaigns$latest_status[[1]] %||% "none"), "-", state$campaigns$remaining_opportunities[[1]] %||% 0L, "remaining,", state$campaigns$knowledge_validated[[1]] %||% 0L, "validated,", state$campaigns$knowledge_conflicts[[1]] %||% 0L, "conflicts")),
