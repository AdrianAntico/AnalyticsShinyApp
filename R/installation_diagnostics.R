...
workstation_installation_info <- function(create = TRUE) {
  paths <- workstation_standard_dirs(create = create)
  ...
  list(
    ...
    app_version = workstation_release_version(),
    paths = as.list(paths),
...
workstation_diagnostics <- function(create = TRUE) {
  ...
  data.table::data.table(
    ...
    status = c(
      ...
      if (dir.exists(paths$app_source)) "available" else "missing",
...
qa_package_distribution <- function() {
  ...
  data.table::data.table(
    ...
    status = c(
      ...
      if (!is.na(workstation_package_resource("electron", "main.js"))) "success" else "error",
...
qa_electron_distribution <- function() {
  ...
  data.table::data.table(
    ...
    status = c(
      ...
      if (!is.na(workstation_package_resource("electron", "main.js"))) "success" else "error",
      if (!is.na(workstation_package_resource("electron", "package.json"))) "success" else "error",
